import Link from "next/link";
import Image from "next/image";
import { MapPin, Mail } from "lucide-react";

export function Footer() {
  return (
    <footer className="bg-green-900 text-white">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          {/* COL 1 — Logo + description */}
          <div className="space-y-4">
            <Image
              src="/assets/logos/logo-blanc-footer.png"
              alt="Tennis Club Halluin"
              width={80}
              height={80}
              className="h-16 w-auto"
            />
            <p className="text-sm text-white/70 leading-relaxed">
              L&apos;excellence du tennis au c&oelig;ur d&apos;Halluin. Un cadre unique pour votre passion, ouvert &agrave; tous.
            </p>
          </div>

          {/* COL 2 — Contact */}
          <div>
            <div className="flex items-center gap-2 mb-5">
              <span className="w-6 h-px bg-[#f6ca73]" />
              <h3 className="text-sm font-bold uppercase tracking-widest text-[#f6ca73]">
                Contact
              </h3>
            </div>
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                <MapPin size={16} className="text-white/50 mt-0.5 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-white">Localisation</p>
                  <p className="text-sm text-white/70">341 rue de la Lys, Halluin</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Mail size={16} className="text-white/50 mt-0.5 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-white">Email</p>
                  <p className="text-sm text-white/70">contact@tch.fr</p>
                </div>
              </div>
            </div>
          </div>

          {/* COL 3 — Explorer */}
          <div>
            <div className="flex items-center gap-2 mb-5">
              <span className="w-6 h-px bg-[#f6ca73]" />
              <h3 className="text-sm font-bold uppercase tracking-widest text-[#f6ca73]">
                Explorer
              </h3>
            </div>
            <ul className="space-y-2.5">
              {[
                { href: "/", label: "Accueil" },
                { href: "/le-club", label: "Le Club" },
                { href: "/reservation", label: "Réservation" },
                { href: "/contact", label: "Contact" },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-white/70 hover:text-white transition-colors flex items-center gap-2"
                  >
                    <span className="text-white/30">&rsaquo;</span>
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* COL 4 — Horaires */}
          <div>
            <div className="flex items-center gap-2 mb-5">
              <span className="w-6 h-px bg-[#f6ca73]" />
              <h3 className="text-sm font-bold uppercase tracking-widest text-[#f6ca73]">
                Horaires
              </h3>
            </div>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-white/70">Semaine</span>
                <span className="text-sm font-semibold text-white">9h - 22h</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-white/70">Weekend</span>
                <span className="text-sm font-semibold text-white">9h - 20h</span>
              </div>
            </div>
          </div>
        </div>

        {/* Barre du bas */}
        <div className="mt-10 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-white/50">
            <span>&copy; {new Date().getFullYear()} Tennis Club d&apos;Halluin</span>
            <span className="text-white/20">&bull;</span>
            <Link href="/mentions-legales" className="hover:text-white/80 transition-colors uppercase tracking-wider font-semibold">
              Mentions l&eacute;gales
            </Link>
          </div>
          <div className="flex gap-3">
            <a
              href="https://facebook.com/tennisclubhalluin"
              target="_blank"
              rel="noopener noreferrer"
              className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
              aria-label="Facebook"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a
              href="https://instagram.com/tch_halluin"
              target="_blank"
              rel="noopener noreferrer"
              className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
              aria-label="Instagram"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}
